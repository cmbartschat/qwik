...
You want to run some code ONLY when the component mounts, once.
...
- **Trigger:** component mount
- **When:** before rendering and resources
- **Times:** exactly once per component instance, or never if the component was present in the initial page load
...
export const Cmp = component$(() => {
  ...
  useClientMount$(() => {
    // This code will ONLY run once in the browser, if useServerMount$ didn't already run.
...
> Notice that `useWatch$()` runs BEFORE the actual rendering and in the server, so manual DOM manipulation needs to be done carefully.
...
You want to mutate some state, or perform some action when some state changes. In other words, you need side effects.

> Note, if you want to load some data (for example a fetch()), to later use that in your component, look at [`useResource$()`](/docs/components/resource/index.mdx). This API can more efficiently leverage SSR streaming and parallelism.
...
For all components that got mounted during SSR (in the server), the `useClientEffect$()` will run eagerly - without user interaction, but the eagerness can be configured!
...
> **NOTE:** Don't abuse `useClientEffect$()` when the same logic can be achieved using `useWatch$()` or other means. Ask yourself: Does this code really need to run in the browser, right at the beginning? If the answer is no, `useClientEffect$()` is probably not the right answer.
